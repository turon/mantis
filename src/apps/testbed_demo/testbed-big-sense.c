...
uint16_t lasttemp, lasthumid, lastlight1, lastlight2;

uint8_t display;
mos_sem_t button;
mos_sem_t recieve;

mos_mutex_t displaym;
...
void sense_thread()
{
  ...
  while(1) {
      ...
      i=(i+1) & 7;
      mos_thread_sleep(7000);// 1/8 of a minute.
...
void recieve_thread()
{
  ...
  while(1)
    {
      mos_led_toggle(0);
      buf = com_recv(IFACE_RADIO);
      com_free_buf(buf);
      mos_sem_post(&recieve);
    }
}
...
void start (void)
{
  ...
  mos_sem_init(&recieve,0);
  mos_mutex_init(&displaym);
  display = 0;

  printf("SYSTEM: NODE: Booted up");
  
  mos_enable_user_button(button_handler);
 
  mos_thread_new (display_thread, 128, PRIORITY_NORMAL);
  mos_thread_new (button_thread, 128, PRIORITY_NORMAL);
  mos_thread_new (sense_thread, 128, PRIORITY_NORMAL);
  mos_thread_new (recieve_thread,128, PRIORITY_NORMAL);
}
